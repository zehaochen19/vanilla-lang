...
-- | Nat declaration
natDec =
  Declaration "Nat" [] [Constructor "Zero" [], Constructor "Succ" [tdata "Nat"]]

-- | Bool declaration
boolDec = Declaration "Bool" [] [Constructor "True" [], Constructor "False" []]
...
-- | Product delaration
prodDec =
  Declaration "Prod" ["a", "b"] [Constructor "Prod" [TVar "a", TVar "b"]]
...
letNestedIdUnit = Program [unitDec] $ ELet "myNestedId" nestedId $ ELet
  "myid"
  id'
  (EVar "myNestedId" $$ EVar "myid" $$ cons "Unit")

illtypedLetNestedUnit :: Program
illtypedLetNestedUnit = Program [unitDec]
  $ ELet "nestedId" nestedId (EVar "nestedId" $$ cons "Unit" $$ cons "Unit")
...
lambdaIdIdUnit =
  Program [unitDec] $ ELam "f" (ELam "x" $ EVar "f" $$ EVar "x") $$ id' $$ cons
    "Unit"

applyToUnit =
  ELam "f" (EVar "f" $$ cons "Unit")
...
ifElseIdNat :: Expr
ifElseIdNat =
  ELet "if" if' $ EVar "if" $$ cons "True" $$ ELam "x" (EVar "x") $$ EALam
    "x"
    (tdata "Nat")
    (EVar "x")
...
nonZero :: Expr
nonZero = EALam "n" (tdata "Nat") $ ECase
  (EVar "n")
  [Branch "Zero" [] $ cons "False", Branch "Succ" ["n2"] $ cons "True"]
...
natPred =
  EALam
      ...
      (tdata "Nat")
      (ECase (EVar "n")
             [Branch "Zero" [] $ cons "Zero", Branch "Succ" ["x"] $ EVar "x"]
      )
    -:  tdata "Nat"
    --> tdata "Nat"
...
aLetId :: Program
aLetId = Program [boolDec] $ EALet "id"
                                   (TAll "A" (TVar "A" --> TVar "A"))
                                   (ELam "x" $ EVar "x")
                                   (EVar "id" $$ cons "True")

boolNatProd :: Expr
boolNatProd = cons "Prod" $$ cons "True" $$ (cons "Succ" $$ cons "Zero")

idProd :: Program
idProd = Program [prodDec, boolDec, natDec] $ EALet
  "id"
  (TAll "A" (TVar "A" --> TVar "A"))
  (ELam "x" $ EVar "x")
...
boolNatProj1 = Program [boolDec, prodDec, natDec]
  $ ECase boolNatProd [Branch "Prod" ["x", "y"] $ EVar "x"]

sumUnit :: Expr
sumUnit = ELam "s" (cons "Unit")
...
mapProgram :: Program
mapProgram = Program [listDec] $ EALetRec
  "map"
  (   TAll "a"
  $   TAll "b"
  $   (TVar "a" --> TVar "b")
