...
evalSpec = describe "eval" $ do
  ...
  it "evals annotedIdSZero" $ eval' annotedIdSZero `shouldBe` cons'
    "Succ"
    [cons "Zero"]
  ...
  it "evals idProd" $ eval' idProd `shouldBe` cons'
    "Prod"
    [cons "False", cons "Zero"]
  it "evals boolNatProj1" $ eval' boolNatProj1 `shouldBe` cons "True"
  it "evals (isInj1 inj2Unit)" $ eval (isInj1 $$ inj2Unit) `shouldBe` cons
    "False"
  it "evals (isInj1 inj1Nat)" $ eval (isInj1 $$ inj1Nat) `shouldBe` cons "True"
  it "evals listEmptyProg" $ eval (mainExpr listEmptyProg) `shouldBe` cons
    "True"
  it "evals listNonEmptyProg" $ eval (mainExpr listNonEmptyProg) `shouldBe` cons
    "False"
