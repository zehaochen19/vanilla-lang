...
branchPSpec = describe "branchP should" $ do
  it "parse a Zero branch" $ runParser branchP "" "Zero → Unit" `shouldBe` Right
...
constructorPSpec = describe "constructorP should" $ do
  it "parse Nil constructor" $ runParser constructorP "" "Nil" `shouldBe` Right
    (Constructor "Nil" [])
...
typeParserSpec = describe "typeP should" $ do
  ...
  it "parse a List type" $ runParser typeP "" "List a" `shouldBe` Right
    (tdata' "List" [TVar "a"])
  it "parse map type"
...
expressionParseSpec = describe "exprP should" $ do
  ...
  it "parse annotated nat id"
    ...
    `shouldBe` Right
                 (EAnno (EALam "x" (tdata "Nat") $ EVar "x")
                        (tdata "Nat" --> tdata "Nat")
                 )
  ...
  it "parse nat 1" $ runParser exprP "" "Succ Zero" `shouldBe` Right
    (cons "Succ" $$ cons "Zero")
  ...
  it "parse a recursive function"
    ...
    `shouldSatisfy` isRight
  it "parse a product" $ runParser exprP "" "Prod True Zero" `shouldBe` Right
  ...
  it "parse a cons" $ runParser exprP "" "Cons Unit Nil" `shouldBe` Right
